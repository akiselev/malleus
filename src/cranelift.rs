...
impl JITCompiler {
    ...
    pub fn compile_newton_step(
        ...
        // SAFETY: `ptr` is the finalized code address for `func_id`, which was
        // defined above with exactly this signature (three pointer params, one
        // f64 return, C ABI). Transmuting the raw code pointer to a matching
        // `extern "C"` fn pointer is sound because the ABI and arity agree.
    ...
    pub fn compile(&mut self, compiled: &CompiledConstraints) -> Result<JITFunction, JITError> {
        ...
        // SAFETY: each `*_ptr` is the finalized code address for the function
        // just defined with the matching signature — `evaluate_residuals` /
        // `evaluate_jacobian` take (*const f64, *mut f64) and the two fused
        // functions take (*const f64, *mut f64, *mut f64), all C ABI. Transmuting
        // each raw code pointer to its corresponding `extern "C"` fn pointer is
        // sound because ABI and arity agree.
        Ok(JITFunction {
...
impl JITFunction {
    ...
    pub fn evaluate_residuals(&self, vars: &[f64], residuals: &mut [f64]) {
        ...
        // SAFETY: `residual_fn` is a JIT'd `extern "C" fn(*const f64, *mut f64)`.
        // The debug_asserts above guarantee `vars.len() >= n_vars` and
        // `residuals.len() >= n_residuals`, which bound every load/store the
        // compiled body performs, so both pointers address valid memory.
        unsafe {
    ...
    pub fn evaluate_jacobian(&self, vars: &[f64], values: &mut [f64]) {
        ...
        // SAFETY: `jacobian_fn` is a JIT'd `extern "C" fn(*const f64, *mut f64)`.
        // The debug_asserts above guarantee `vars.len() >= n_vars` and
        // `values.len() >= jacobian_nnz`, bounding every load/store the compiled
        // body performs, so both pointers address valid memory.
        unsafe {
    ...
    pub fn evaluate_both(&self, vars: &[f64], residuals: &mut [f64], jacobian_values: &mut [f64]) {
        ...
        // SAFETY: `fused_fn` is a JIT'd
        // `extern "C" fn(*const f64, *mut f64, *mut f64)`. The debug_asserts
        // above guarantee `vars`, `residuals`, and `jacobian_values` are each at
        // least `n_vars` / `n_residuals` / `jacobian_nnz` long, bounding every
        // load/store the compiled body performs.
        unsafe {
            (self.fused_fn)(
                vars.as_ptr(),
    ...
    pub fn evaluate_both_dense(
        ...
        // SAFETY: `dense_fused_fn` is a JIT'd
        // `extern "C" fn(*const f64, *mut f64, *mut f64)`. The debug_asserts
        // above guarantee `vars.len() >= n`, `residuals.len() >= m`, and
        // `dense_jacobian.len() >= m * n`; the dense buffer is zeroed just above
        // and the compiled body only writes the pattern's non-zero entries, all
        // within `m * n`.
        unsafe {
            (self.dense_fused_fn)(
                vars.as_ptr(),
