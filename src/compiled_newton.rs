...
impl CompiledNewtonStep {
    ...
    pub fn evaluate(&self, vars_in: &[f64], vars_out: &mut [f64], scratch: &mut [f64]) -> f64 {
        ...
        // SAFETY: `step_fn` is a Cranelift-JIT'd function whose signature is
        // `fn(*const f64, *mut f64, *mut f64) -> f64` (established at compile
        // time in `JITCompiler::compile_newton_step`). The debug_asserts above
        // guarantee the three buffers are each at least the length the compiled
        // code indexes (`n`, `n`, and `n + n*n + n`), so every load/store the
        // JIT'd body performs stays in bounds. This is the JIT call boundary —
        // the one legitimate unsafe site in the crate.
        let norm = unsafe {
            (self.step_fn)(
                vars_in.as_ptr(),
                vars_out.as_mut_ptr(),
